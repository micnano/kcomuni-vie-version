...
{if $module_result.path|count()|ge(4)}
					<li{if $current_node_id|eq($module_result.path.[1].node_id)} class="active"{/if}>{if $current_node_id|eq($module_result.path.[1].node_id)}<span class="selected"></span>{/if}<a href={$module_result.path.[1].url_alias|ezurl()}>{$module_result.path.[1].text}</a></li>
