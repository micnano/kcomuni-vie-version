...
{if $use_template_search}
	...
	{if ezhttp_hasvariable( 'PathId', 'get' )}
		{set $tree=array(ezhttp( 'PathId', 'get' ))}
	{/if}
    ...
    {def $filterParameters = fetch( 'ezfind', 'filterParameters' )
         $defaultSearchFacets = fetch( 'kfunctions', 'getComuniSearchFacets' )}
...
{if ezhttp_hasvariable( 'PathId', 'get' )}
		<input type="hidden" name="PathId" value="{ezhttp( 'PathId', 'get' )}" />
	{/if}
...
{/switch}
  ...
  <div id="search_results">
    {include name=Navigator
             ...
             page_uri_suffix=concat('?SearchText=',$search_text|urlencode, ezhttp_hasvariable( 'PathId', 'get' )|choose('', concat('&PathId=', ezhttp( 'PathId', 'get' ))),$search_timestamp|gt(0)|choose('',concat('&SearchTimestamp=',$search_timestamp)), $uriSuffix )
...
<div class="clearfix"></div>
    ...
    {foreach $search_result as $result sequence array(bglight,bgdark) as $bgColor}
		{node_view_gui view='line' sequence=$bgColor use_url_translation=$use_url_translation content_node=$result q=$search_text}
             {set $num=$num|inc(1)}
