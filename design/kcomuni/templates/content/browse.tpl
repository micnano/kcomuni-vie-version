...
{else}
    ...
    <h1 class="long">{'Browse'|i18n( 'design/ezdemo/content/browse' )} - {$main_node.name|wash}</h1>
    </div>
...
{if $browse.start_node|gt( 1 )}
    ...
    <a href={concat( '/content/browse/', $main_node.parent_node_id, '/' )|ezurl}><img src={'back-button-16x16.gif'|ezimage} alt="{'Back'|i18n( 'design/ezdemo/content/browse' )}" /></a>
    {$current_node.name|wash}&nbsp;[{$current_node.children_count}]</h2>
{else}
    <h2 class="context-title"><img src={'back-button-16x16.gif'|ezimage} alt="Back" />&nbsp;{'Top level'|i18n( 'design/ezdemo/content/browse' )}&nbsp;[{$current_node.children_count}]</h2>
{/if}
...
{/if}
        ...
        <input class="button" type="submit" name="SelectButton" value="{'Select'|i18n('design/ezdemo/content/browse')}" />


{if $cancel_action}
<input type="hidden" name="BrowseCancelURI" value="{$cancel_action|wash}" />
{/if}
 <input class="button" type="submit" name="BrowseCancelButton" value="{'Cancel'|i18n( 'design/ezdemo/content/browse' )}" />
</form>
